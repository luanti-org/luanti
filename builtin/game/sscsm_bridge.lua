...
-- Node defs are immutable after server startup, so the payload is the
-- same for every player. Build once, cache, reuse on every "ready".
local cached_payload
local function get_content_defs_payload()
	if cached_payload then return cached_payload end
	...
	cached_payload = table.concat(parts, ";")
...
core.register_on_clientmodchannel_message(function(channel, sender, msg)
	...
	local payload = get_content_defs_payload()
