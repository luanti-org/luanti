...
describe("constructors", function()
	...
	describe("maps_to", function()
		local x, y, z = vector.new(1, 0, 0), vector.new(0, 1, 0), vector.new(0, 0, 1)
		it("works if the two vectors span a plane", function()
			local rot = Rotation.maps_to(x, z)
			assert_close_vec(z, rot:apply(x))
			assert_close_vec(y, rot:apply(y))
		end)
		-- Edge cases
		it("works if the two vectors point in the same direction", function()
			assert_close_vec(x, Rotation.maps_to(2 * x, 3 * x):apply(x))
		end)
		it("works if the two vectors point in opposite directions", function()
			assert_close_vec(-x, Rotation.maps_to(-2 * x, 3 * x):apply(x))
		end)
	end)
end)
