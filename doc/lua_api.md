...
A mapblock being "loaded" means that is in memory. To reach this state, it must
first be "emerged". This means that it is either loaded from disk, or, if it
doesn't yet exist, generated by the map generator. Mapblocks are loaded in a
broad area around each player. They are "unloaded" again if no player is close
enough. The engine commonly represents the contents of unloaded mapblocks as
`"ignore"` nodes.
