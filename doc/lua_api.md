...
* `number`: A ColorInteger specifying the color used to draw the text
...
* `number:` A ColorInteger for color used to draw the text.
...
A ColorInteger is an integer that specifies a 24-bit color, with each color
component red, green and blue taking one byte.
...
* `core.emerge_area(pos1, pos2, [callback], [param])`
    ...
    * The function signature of callback is:
      `function EmergeAreaCallback(blockpos, action, calls_remaining, param)`
        ...
        * `calls_remaining` is the amount of callbacks to be expected
          after this one.
...
* `set_stars(star_parameters)`:
    ...
    * `star_parameters` is a table with the following optional fields:
        ...
        * `star_seed`: Integer which decides how to generate the sky stars. If set to zero, client picks a random number. (default: `0`)
