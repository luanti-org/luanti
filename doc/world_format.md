...
World metadata.
    ...
    rollback_backend = sqlite3    - which DB backend to use for rollback (sqlite3, postgresql). If unset, defaults to `backend`.
...
`PostgreSQL` backend specific settings:
    ...
    pgsql_rollback_connection = (same parameters as above; if unset, falls back to `pgsql_connection`)
