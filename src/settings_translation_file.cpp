...
fake_function() {
	...
	gettext("In creative mode players are able to dig all kind of blocks nearly\ninstantly, and have access to unlimited resources.\nSome of the functionality is only available if this setting is present\nat startup.");
	gettext("Fire");
	gettext("Flammable nodes will be ignited by nearby igniters. Spreading fire may\ncause severe destruction.\nSpreading fire nodes will disappear when fire is disabled, but\n'permanent_flame' nodes are unaffected.");
	gettext("Flame Sound");
	gettext("Enable flame sound.");
	gettext("Lavacooling");
	gettext("Enable lavacooling.");
	gettext("If enabled, steel tools, torches and cobblestone will be given to new\nplayers.");
	gettext("Give initial items");
	gettext("If enabled, players respawn at the bed they last lay on instead of normal\nspawn.\nThis setting is only read at startup.");
	gettext("Respawn at bed");
	gettext("If enabled, the night can be skipped if more than half of the players are\nin beds.");
	gettext("Skip night when sleeping");
	gettext("If enabled, fences and walls cannot be jumped over.");
	gettext("Tall fences and walls");
	gettext("When TNT explodes, it destroys nearby nodes and damages nearby players.\nThis setting is disabled by default on servers.");
	gettext("TNT");
	gettext("The radius in which nodes will be destroyed by a TNT explosion.");
	gettext("TNT radius");
	gettext("Sets the behaviour of the inventory items when a player dies.\nbones: Store items in a bone node but drop items if inside protected area.\ndrop: Drop items on the ground.\nkeep: Player keeps items.");
	gettext("Bones mode");
	gettext("The time in seconds after which the bones of a dead player can be looted\nby everyone.\nSetting this to 0 will disable sharing of bones completely.");
	gettext("Bones share time");
	gettext("How much earlier the bones of a dead player can be looted by\neveryone if the player dies in a protected area they don't own.\n0 to disable. By default it is "share_bones_time" divide by four.");
	gettext("Earlier bones share time");
	gettext("Inform player of condition and location of new bones.");
	gettext("Inform player about bones");
	gettext("Replaces old stairs with new ones. Only required for older worlds.");
	gettext("Replace old stairs");
	gettext("If enabled, use the engine's spawn search which does not check for a\nsuitable starting biome.");
	gettext("Use engine spawn search");
	gettext("Whether river water source nodes create flowing sounds.\nHelps rivers create more sound, especially on level sections.");
	gettext("River source node sounds");
	gettext("If enabled, the 'weather' mod will control cloud parameters, shadow intensity,\nbloom and volumetric lighting.\nNon-functional in V6 or Singlenode mapgens.");
	gettext("Enable weather");
	gettext("If enabled, non-player actions are logged");
	gettext("Log non-player action");
}
