...
void TestCollision::testCollisionMoveSimple(IGameDef *gamedef)
{
	...
	const auto collide = [gamedef, box](auto extern_env, f32 stepheight, f32 dtime,
		v3f* pos, v3f* speed, v3f accel) {
			return collisionMoveSimple(extern_env, gamedef, box, stepheight, dtime,
	...
	speed = fpos(0, 0, 0);
	accel = fpos(0, 1, 0);
	res = collide(env.get(), 0.0f, 1.0f, &pos, &speed, accel);
	...
	res = collide(env.get(), 0.0f, 0.05f, &pos, &speed, accel);
	...
	speed = fpos(0, -3.f, 0);
	accel = fpos(0, -9.81f, 0);
	res = collide(env.get(), 0.0f, 0.5f, &pos, &speed, accel);
	...
	speed = fpos(0, 2.0f, 0);
	accel = fpos(0, -9.81f, 0);
	res = collide(env.get(), 0.0f, 0.2f, &pos, &speed, accel);
	...
	speed = fpos(0, 0, 0);
	accel = fpos(0, 0, 0);
	res = collide(env.get(), 0.0f, 1/60.0f, &pos, &speed, accel);
	UASSERT(!res.collides);
