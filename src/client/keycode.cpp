...
KeyPress::KeyPress(const SEvent::SMouseInput &in)
{
	...
	case EMIE_OMOUSE_LEFT_UP:
		...
		break;
	default:
		emplace<InputType::MOUSE_BUTTON>(0);
	}
}
...
std::string KeyPress::sym() const
{
	...
	case InputType::KEYBOARD:
		return "SYSTEM_SCANCODE_" + std::to_string(getValue<InputType::KEYBOARD>());
	case InputType::MOUSE_BUTTON:
		return "MOUSE_BUTTON_" + std::to_string(getValue<InputType::MOUSE_BUTTON>());
	default:
		return "";
	}
}
...
std::string KeyPress::name() const
{
	...
	case InputType::MOUSE_BUTTON: {
		auto button = getValue<InputType::MOUSE_BUTTON>();
		switch (button) {
...
KeyPress::operator bool() const
{
	switch (getType()) {
	case InputType::KEYBOARD:
		return getValue<InputType::KEYBOARD>() != 0;
	case InputType::MOUSE_BUTTON:
		return getValue<InputType::MOUSE_BUTTON>() != 0;
	case InputType::GAME_ACTION:
		return getValue<InputType::GAME_ACTION>() < KeyType::INTERNAL_ENUM_COUNT;
	default:
		return false;
	}
}
