...
RenderStep *addPostProcessing(RenderPipeline *pipeline, RenderStep *previousStep, v2f scale, Client *client)
{
	...
	verbosestream << "addPostProcessing(): AA = "
		<< (enable_msaa ? "msaa" : enable_ssaa_smooth ? "ssaa" :
			enable_ssaa_ssim_based ? "ssaa_ssim_based" : "none")
	...
	//~ if (enable_fxaa) {
	if (enable_fxaa && !enable_ssaa_ssim_based) {
	...
	if (enable_ssaa_ssim_based) {
		...
		// TODO: this does not work together with FXAA
		//~ PostProcessingStep *effect = pipeline->createOwned<PostProcessingStep>(shader_id, std::vector<u8>{enable_fxaa ? TEXTURE_AA_OUTPUT : TEXTURE_COLOR});
		PostProcessingStep *effect = pipeline->createOwned<PostProcessingStep>(shader_id, std::vector<u8>{TEXTURE_COLOR});
		pipeline->addStep(effect);
		effect->setRenderSource(buffer);
